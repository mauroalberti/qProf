...
def write_topography_allDEMs_ptshp(fileName, header_list, dem_names, export_data, sr):
    ...
    for dem_ndx in range(len(dem_names)):
        layer.CreateField(ogr.FieldDefn(header_list[3 + dem_ndx * 3 + 1], ogr.OFTReal))
        layer.CreateField(ogr.FieldDefn(header_list[3 + dem_ndx * 3 + 2], ogr.OFTReal))
        layer.CreateField(ogr.FieldDefn(header_list[3 + dem_ndx * 3 + 3], ogr.OFTReal))
...
def write_profile_lnshp(fileName, header_list, points, sr):
...
def write_topography_singleDEM_ptshp(fileName, header_list, export_data, current_dem_ndx, sr):
...
def write_topography_singleDEM_lnshp(fileName, header_list, export_data, current_dem_ndx, sr):
